...
// GET /parent/children — barcha bog'langan farzandlar ro'yxati
export async function listChildren(req, res, next) {
  try {
    const children = await ParentChild.find({ parentId: req.user._id })
...
// GET /parent/children/:studentId/dashboard — farzand umumiy ko'rinishi
export async function getChildDashboard(req, res, next) {
  try {
    ...
    await relation.populate('studentId', 'fullName phone email status');

    const group = await Group.findOne({ studentIds: req.params.studentId })
...
export async function getChildGrades(req, res, next) {
  try {
    ...
    const quizAttempts = await QuizAttempt.find({
      ...
      status: { $in: [QUIZ_ATTEMPT_STATUS.SUBMITTED, QUIZ_ATTEMPT_STATUS.GRADED] },
    })
